...
Safety rules:
1. Work only inside this repository except for the dashboard-owned derived databases at ~/.claude/usage.db and ~/.claude-codex-usage/codex.db.
...
9. Start `<PYTHON> cli.py dashboard --test-mode --no-browser --port 8081`, open http://localhost:8081, and verify the orange TEST MODE banner, blank first-run screen, fake multi-account samples, and Reset preview. Confirm this mode does not invoke provider login, scan local histories, or read normal account profiles.
