...
| macOS Claude Code keychain, legacy `accounts add/refresh` only | Existing private account-orb refresh | OAuth chain in `~/.claude/usage_accounts.json` |
...
The credential-free connectors intentionally do not extract tokens, read
keychains, scrape browsers, or call private Anthropic endpoints. The separate
legacy account-orb commands retain their existing keychain behavior and are
never used by the credential-free profile workflow.
...
```bash
python3 cli.py accounts profiles add --id max-one --label "Max account one" \
  --claude-email you@example.com --codex-email you@example.com --tier "Max 20x"
python3 cli.py accounts profiles snapshot --profile max-one --provider all
python3 cli.py accounts profiles list
```
...
```bash
python3 cli.py dashboard --test-mode --port 8334
```
...
```bash
python3 cli.py accounts profiles --testing samples
python3 cli.py accounts profiles --testing reset --yes
```
...
```bash
python3 cli.py accounts profiles --testing add --id studio --label "Studio Max" \
...
```bash
python3 cli.py accounts add
python3 cli.py accounts list
python3 cli.py accounts remove you@example.com
python3 cli.py accounts refresh
```

Those legacy commands read the active Claude Code keychain entry and persist an
OAuth refresh chain in `~/.claude/usage_accounts.json`. They are optional and
are never invoked by `accounts setup`, `accounts profiles ...`, or test mode.
