# Graph Report - claude-usage  (2026-07-24)

## Corpus Check
- 62 files · ~126,493 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1114 nodes · 1828 edges · 68 communities (62 shown, 6 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 7 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `7e1af6ae`
...
2. `get_pricing()` - 26 edges
3. `_daemon()` - 26 edges
4. `scan()` - 23 edges
5. `TestGetPricing` - 21 edges
6. `get_dashboard_data()` - 19 edges
7. `parse_jsonl_file()` - 19 edges
8. `TestUsageCooldowns` - 19 edges
...
- `cost_for_prefix()` --calls--> `calc_cost()`  [EXTRACTED]
...
Cohesion: 0.16
Nodes (3): TestCardReport, TestSparkReport, TestTableReport
...
Cohesion: 0.29
Nodes (5): Tests for views.py — fetch_period_data and display functions., Display labels must survive new major versions (Opus 5, Sonnet 5, ...)., TestModelShort, _model_short(), Return a short display label for a model string.
...
Cohesion: 0.18
Nodes (12): Connection, _cache_savings(), card_report(), _date_range(), views.py - Display modes for the claude-usage report command.  Public API:     f, Estimate dollars saved by cache reads vs paying full input price., Print a tabular usage report to stdout., Return (start_iso, end_iso) for a period string, or (None, None) for 'all'. (+4 more)

### Community 22 - "Community 22"
Cohesion: 0.21
Nodes (16): cmd_daemons(), cmd_dashboard(), cmd_fable_cost(), cmd_report(), cmd_scan(), cmd_stats(), cmd_today(), cmd_week() (+8 more)
...
Cohesion: 0.11
Nodes (34): cmd_fable_next(), _fable_rank(), _fmt_reset_local(), _norm_discount(), Render a UTC reset timestamp in local time as 'Jul 08 05:59', or '--'., Score one dashboard entry for Fable-5 suitability.      Returns a dict with fabl, Snapshot the live Claude Code keychain into the store as the new owner.      Run, Parse a discount arg into a 0..1 fraction. Accepts '30', '30%', or '0.3'. (+26 more)
